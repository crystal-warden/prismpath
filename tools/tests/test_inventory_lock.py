...
lock entry is reported. The real inventory must classify the real tree completely."""
...
def _rules(tmp_path):
    ...
    return product_inventory.load_rules(inventory)


def test_narrow_rule_wins_over_broad_prefix(tmp_path):
    rules = _rules(tmp_path)
    assert product_inventory.classify("prismpath/kernel/engine.py", rules).identifier == "engine"
    assert product_inventory.classify("prismpath/comparisons/x.json", rules).identifier == "held"
    assert product_inventory.classify("README.md", rules).identifier == "one-file"
    assert product_inventory.classify("elsewhere.txt", rules) is None
...
def test_unlocked_and_held_paths_are_unclassified(tmp_path):
    ...
    findings = dict(product_inventory.unclassified(paths, rules, lock))
...
def test_stale_lock_entries_are_reported():
    ...
    assert product_inventory.stale_lock_entries(["present.py"], lock) == ["gone.py"]


def test_lock_round_trips(tmp_path):
    lock_path = tmp_path / "inventory.lock"
    ...
    product_inventory.write_lock(entries, lock_path)
    assert list(product_inventory.load_lock(lock_path)) == ["a.py", "b.py"]
    assert product_inventory.load_lock(lock_path)["b.py"]["source_blob"] == "abc"
    assert product_inventory.load_lock(lock_path)["b.py"]["adopted_revision"] == "f" * 40


def test_every_adopted_path_records_its_revision():
    for entry in product_inventory.load_lock().values():
        assert bool(entry["source_blob"]) == bool(entry["adopted_revision"]), entry["path"]


def test_the_real_tree_is_fully_classified():
    rules = product_inventory.load_rules()
    lock = product_inventory.load_lock()
    paths = product_inventory.tracked_paths()
    assert product_inventory.unclassified(paths, rules, lock) == []
    assert product_inventory.stale_lock_entries(paths, lock) == []
