# prismpath/telemetry · decision-preserving telemetry
...
at all.
```
python prismpath/telemetry/facet_init.py <sample.ndjson> \
...
## Tests
```
python -m pytest prismpath/telemetry
```
The gates are standalone (not pytest); rerun them to reproduce the numbers cited above:
```
python prismpath/telemetry/bench/run.py            # Phase A go/no-go -> bench/results.md
