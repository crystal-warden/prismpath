...
"""Preflight reports the input contract's rejections field by field and withholds READY on them,
and a clean sample that the contract accepts is READY with an empty rejection map, so a passing
...
def test_contract_rejections_are_reported_and_block_ready():
    report = _scan(['{"temp": 95, "armed": true}', '{"temp": "hot", "armed": true}', '{"temp": 2.5, "armed": "yes"}'])
    assert report["rejected_by_contract"] == {"armed": {"wrong_type": 1}, "temp": {"unparseable_string": 1, "fractional": 1}}
    assert report["out_of_partition"] == {"temp": 1}
    assert report["ready"] is False


def test_clean_sample_has_no_rejections_and_is_ready():
    report = _scan(['{"temp": 95, "armed": true}', '{"temp": 10, "armed": false}', '{"temp": "60", "armed": 1}'])
    assert report["rejected_by_contract"] == {}
