...
def salt_leaf(leaf_hex, secret):
    ...
    return hmac.new(
        secret.encode() if isinstance(secret, str) else secret, bytes.fromhex(leaf_hex), hashlib.sha256
    ).hexdigest()


def provenance_manifest(
    root_hex, label, policy_hash=None, gate_id=None, ingestion_hashes=None, knowledge_base_hash=None
):
...
def override_manifest(prior, overrider_id, rationale, new_root_hex, new_label=None):
    ...
    manifest = {
        ...
        "rationale": rationale,
        "root": new_root_hex,
        "label": new_label or ("override:" + str(prior.get("label", ""))),
        "created": _now_iso(),
...
def make_test_tsa(dirpath):
    ...
    tsa_key = os.path.join(dirpath, "tsa.key")
    tsa_csr = os.path.join(dirpath, "tsa.csr")
