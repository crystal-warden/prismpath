...
def test_baseline_passes_every_proof():
    ...
    assert all(proof["ok"] for proof in res["proofs"].values())
...
def test_downgrade_trips_class_floor_only():
    proofs = _prove("downgrade_cui_to_classical")["proofs"]
    assert proofs["P3_class_floor"]["ok"] is False
    assert proofs["P1_envelope_closure"]["ok"] is True and proofs["P5_decidable"]["ok"] is True


def test_classical_past_phase_floor_trips_monotone_migration():
    proofs = _prove("classical_past_phase_floor")["proofs"]
    assert proofs["P4_monotone_migration"]["ok"] is False
    assert any(violation["suite"] == "tls13-aesgcm" for violation in proofs["P4_monotone_migration"]["violations"])


def test_unapproved_suite_trips_envelope_closure():
    proofs = _prove("unapproved_suite")["proofs"]
    assert proofs["P1_envelope_closure"]["ok"] is False
    assert any(offender["suite"] == "rot13-lol" for offender in proofs["P1_envelope_closure"]["offenders"])


def test_non_total_router_trips_totality():
    proofs = _prove("non_total_router")["proofs"]
    assert proofs["P2_totality"]["ok"] is False
    assert "classify" in proofs["P2_totality"]["nodes_without_catchall"]
