...
def test_no_tests_is_invalid(tmp_path):
    ...
    report = pysprint.validate(str(tmp_path))
    assert report["valid"] is False
    assert any("no test" in error.lower() for error in report["errs"])
...
def test_passing_suite_is_green(tmp_path):
    ...
    assert report["errs"] == []
    assert report["biggest_file"]                 # reports a size like the browser gate
...
def test_failing_test_names_the_file(tmp_path):
    ...
    report = pysprint.validate(str(tmp_path))
    assert report["valid"] is False
    assert any("test_mod.py" in error for error in report["errs"])
...
def test_import_error_is_reported_not_raised(tmp_path):
    ...
    report = pysprint.validate(str(tmp_path))
    assert report["valid"] is False
    assert report["errs"]                         # some diagnostic, never an exception
