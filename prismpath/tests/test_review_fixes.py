...
def test_utf8_flow_parses(tmp_path):
    ...
    path.write_text("---\nstart: a\n---\n## a\nDo — a thing with ünïcode.\n-> b: when always\n## b\n",
                 encoding="utf-8")
    assert parse_file(str(path)).nodes["a"].instruction.startswith("Do —")
...
def test_prefilter_migrate_defaults_action(tmp_path):
    cache = prefilter.PrefilterCache(tmp_path / "c", embed_fn=lambda texts: np.ones((len(texts), 3), "float32"))
    import json
...
def test_ledger_gate_red_does_not_head_of_line_block(tmp_path):
    ...
    class Worker:
        def __call__(self, node, instr, state):
            if node == "fetch":
                ...
                nxt = next((item for item in ["bad", "good"] if item not in done), None)
                if nxt is None:
    ...
    committed = run_ledgered_loop(str(flow), Worker(), led)
...
def test_double_resume_keeps_full_path(tmp_path):
    ...
    class CrashAt:
        ...
        def __call__(self, node, instruction, state):
            if node == self.at: raise RuntimeError("x")
    ...
    assert [step.node for step in res.steps] == ["a", "b", "c", "d"]        # steps consistent with path
